...
const Signup = () => {
  ...
  const postDetails = (pics) => {
    ...
    if (pics.type === "image/jpeg" || pics.type === "image/png") {
      ...
      data.append("cloud_name", "dxklsmcmf")
      fetch("https://api.cloudinary.com/v1_1/dxklsmcmf/upload", {
        method: "POST",
        body: data,
      }).then(res => res.json())
        .then(data => {
  ...
  return (
    <VStack spacing="5px">
      ...
      <FormControl id='password' isRequired>
        ...
        <InputGroup>
          <Input type={show ? "text" : "password"} placeholder='Confirm Password' onChange={(event) => { setConfirmPassword(event.target.value) }} />
          <InputRightElement width={"4.5rem"}>
            <Button h="1.75rem" size="sm" onClick={e => setShow(!show)}>
              {show ? "Hide" : "Show"}
            </Button>
